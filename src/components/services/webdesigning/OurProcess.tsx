import React from 'react';
import {
  ClipboardCheck, Layout as LayoutIcon, Palette,
  Code2, SearchCheck, Rocket, Headset
} from 'lucide-react';
import ProcessStep from '@/components/ProcessStep'; // Adjust the import path as needed

// const steps = [
//   { title: "Requirement", description: "Understanding your business goals.", icon: <ClipboardCheck className="w-6 h-6" /> },
//   { title: "Planning", description: "Mapping out the site structure.", icon: <LayoutIcon className="w-6 h-6" /> },
//   { title: "Designing", description: "Creating the visual brand identity.", icon: <Palette className="w-6 h-6" /> },
//   { title: "Development", description: "Building with clean, scalable code.", icon: <Code2 className="w-6 h-6" /> },
//   { title: "Testing", description: "Ensuring a bug-free experience.", icon: <SearchCheck className="w-6 h-6" /> },
//   { title: "Launch", description: "Making your website live.", icon: <Rocket className="w-6 h-6" /> },
//   { title: "Support", description: "Ongoing updates and help.", icon: <Headset className="w-6 h-6" /> },
// ];

const steps = [
  { title: "Discovery", description: "Analyzing your business needs and objectives.", icon: <ClipboardCheck className="w-6 h-6" /> },
  { title: "Strategy", description: "Planning the website structure and flow.", icon: <LayoutIcon className="w-6 h-6" /> },
  { title: "Design", description: "Crafting the visual look and feel of your brand.", icon: <Palette className="w-6 h-6" /> },
  { title: "Development", description: "Implementing clean and scalable code.", icon: <Code2 className="w-6 h-6" /> },
  { title: "Quality Check", description: "Testing thoroughly for errors and performance.", icon: <SearchCheck className="w-6 h-6" /> },
  { title: "Deployment", description: "Launching your website to the public.", icon: <Rocket className="w-6 h-6" /> },
  { title: "Maintenance", description: "Providing continuous support and updates.", icon: <Headset className="w-6 h-6" /> },
];

export default function OurProcess() {
  return (
    <section className="py-20 bg-white">
      <div className="container mx-auto px-6">
        <h2 className="text-3xl md:text-4xl font-bold mb-20 text-center text-slate-900">
          Our<span className="text-red-500"> Website Design</span> Workflow
        </h2>

        <div className="flex flex-col md:flex-row justify-between items-start gap-12 relative">
          {/* The Progress Line (Only visible on desktop) */}
          <div className="hidden md:block absolute top-8 left-10 right-10 h-0.5 bg-slate-200 z-0" />

          {steps.map((step, i) => (
            <ProcessStep
              key={i}
              index={i}
              title={step.title}
              description={step.description}
              icon={step.icon}
            />
          ))}
        </div>
      </div>
    </section>
  );
}